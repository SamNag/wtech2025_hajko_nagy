<header class="px-4 pb-4 bg-gray-200 fixed top-0 left-0 w-full border border-gray-300 z-50">
    <nav class="flex flex-grow justify-start md:justify-between items-center space-x-8 mt-4 ml-4">
        ...
        <div class="hidden md:flex items-center space-x-8">
            <a href="{{ route('products') }}" class="inconsolata-regular hover:text-gray-500 text-xl">Products -</a>
            <a href="{{ route('about') }}" class="inconsolata-regular hover:text-gray-500 text-xl">About -</a>
        ...
        <div class="hidden md:flex flex items-center space-x-4 mr-2">
            ...
            <a href="{{ route('cart') }}"
               class="classic-clicked flex items-center justify-center text-gray-600 font-bold rounded-lg h-12 w-12 relative">
                ...
                @else
                    <span id="cart-count" class="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center hidden">
                        0
                    </span>
                @endif
            </a>
                ...
                <ul class="font-medium flex flex-col items-center p-4 md:p-0 mt-4 bg-gray-200 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:bg-gray-200">
                    ...
                    <li>
                        <a href="{{ route('cart') }}" class="inconsolata-regular hover:text-gray-500 text-xl relative">
                            Cart
                            @if($cartCount > 0)
                                <span class="bg-red-500 text-white text-xs rounded-full h-5 w-5 inline-flex items-center justify-center ml-1">
                                    {{ $cartCount }}
                                </span>
                            @endif
                        </a>
